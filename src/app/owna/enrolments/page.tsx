...
export default function OwnaEnrolmentsPage() {
  ...
  const [tab, setTab] = useState<'submissions' | 'enquiries' | 'waitlist' | 'tours'>('submissions')
    ...
    const load = async () => {
      try {
        const [subRes, enqRes, wlRes, tourRes] = await Promise.all([
          ...
          ownaFetch(`/api/enquiries/${DEMO_CENTRE_ID}/list`),
          ownaFetch(`/api/waitlist/${DEMO_CENTRE_ID}/list`),
  ...
  const pipelineTotal = enquiries.length + waitlist.length + submissions.length
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Enquiries, waitlist, tour bookings, and enrolment submissions from OWNA</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 relative overflow-hidden">
          ...
          <p className="text-xs text-gray-400 mt-1">Initial contact</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 relative overflow-hidden">
      ...
      {pipelineTotal > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
          <h2 className="font-semibold text-gray-900 text-sm mb-3">Pipeline Overview</h2>
      ...
      {/* Data table — generic renderer */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {(() => {
          const data = tab === 'submissions' ? submissions : tab === 'enquiries' ? enquiries : tab === 'waitlist' ? waitlist : tourBookings
          if (data.length === 0) return <div className="py-12 text-center text-gray-400 text-sm">No {tab} data available</div>

          const keys = Object.keys(data[0]).filter(k => !k.startsWith('_') && k !== 'id' && typeof data[0][k] !== 'object')
                      ...
                      {displayKeys.map(k => {
                        ...
                        return (
                          <td key={k} className="py-2.5 px-3 text-gray-600 text-xs truncate max-w-[200px]">
                            {isDate ? new Date(val).toLocaleDateString() : String(val ?? '-')}
                          </td>
                        )
                      })}
                    </tr>
