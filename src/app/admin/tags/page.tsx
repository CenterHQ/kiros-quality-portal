...
export default function TagsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-gray-500 text-sm mb-6">Manage tags used across elements, tasks, and actions</p>

      {/* Add Tag */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <h3 className="font-semibold text-sm text-gray-700 mb-3">Add New Tag</h3>
        <div className="flex gap-3 items-end">
          <div className="flex-1">
            <label className="block text-xs text-gray-500 mb-1">Name</label>
            <input type="text" value={newTag.name} onChange={e => setNewTag({...newTag, name: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#470DA8] outline-none" placeholder="e.g. Needs Follow-up" />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={newTag.category} onChange={e => setNewTag({...newTag, category: e.target.value})}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#470DA8] outline-none">
              {CATEGORIES.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
            </select>
          </div>
          <button onClick={addTag} className="px-4 py-2 bg-[#470DA8] text-white rounded-lg text-sm font-medium hover:opacity-90">Add</button>
        </div>
      </div>
      ...
      {CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-4">
            <div className="px-5 py-3 border-b border-gray-200 bg-gray-50">
              <h3 className="font-semibold text-sm text-gray-700">{cat.charAt(0).toUpperCase() + cat.slice(1)} Tags</h3>
            </div>
            <div className="p-4 flex flex-wrap gap-2">
              {catTags.map(tag => (
                <div key={tag.id} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-200 bg-white">
                  ...
                  <button onClick={() => deleteTag(tag.id, tag.name)} className="text-gray-400 hover:text-red-500 text-xs ml-1">&times;</button>
                </div>
              ))}
            </div>
          </div>
