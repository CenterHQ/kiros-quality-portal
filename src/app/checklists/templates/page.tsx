        ...
        <div className="bg-card rounded-xl shadow-sm border border-border p-6 mb-6">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {item.type === 'dropdown' && (
                      <div>
                        ...
                        <input type="text" value={(item.options || []).join(', ')} onChange={e => updateItem(index, { options: e.target.value.split(',').map(o => o.trim()).filter(Boolean) })} placeholder="Option 1, Option 2, Option 3" className="w-full px-3 py-1.5 border border-border rounded-lg text-xs focus:ring-2 focus:ring-primary focus:border-transparent" />
                      </div>
                    )}
                      ...
                      <div className="flex items-center gap-4 text-xs">
                        ...
                        {builderItems.filter(i => i.type === 'yes_no' && i.id !== item.id).length > 0 && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <select value={item.conditional_on || ''} onChange={e => updateItem(index, { conditional_on: e.target.value || undefined, conditional_value: e.target.value ? true : undefined })} className="px-2 py-1 border border-border rounded text-xs">
                            ...
                            {item.conditional_on && (
                              <select value={String(item.conditional_value)} onChange={e => updateItem(index, { conditional_value: e.target.value === 'true' })} className="px-2 py-1 border border-border rounded text-xs">
          ...
          {builderItems.length > 0 && (
            <div className="flex gap-2 mt-4 pt-4 border-t border-border">
              ...
              <button onClick={() => addItem('photo')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Photo</button>
              <button onClick={() => addItem('dropdown')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Dropdown</button>
              <button onClick={() => addItem('signature')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Signature</button>
              <button onClick={() => addItem('date')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Date</button>
              <button onClick={() => addItem('time')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Time</button>
              <button onClick={() => addItem('heading')} className="px-3 py-1.5 border border-dashed border-border text-muted-foreground rounded-lg text-xs hover:border-primary hover:text-primary transition">+ Section</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        {isPrivileged && (
          <button onClick={() => openBuilder()} className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:opacity-90">
            + New Template
          </button>
        )}
      </div>
      ...
      <div className="flex gap-3 mb-6">
        <select value={filter.category} onChange={e => setFilter({ ...filter, category: e.target.value })} className="px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
          <option value="">All Categories</option>
          {categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
        </select>
        <select value={filter.frequency} onChange={e => setFilter({ ...filter, frequency: e.target.value })} className="px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
          <option value="">All Frequencies</option>
          {Object.entries(CHECKLIST_FREQUENCY_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <select value={filter.status} onChange={e => setFilter({ ...filter, status: e.target.value })} className="px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                  ...
                  {t.related_qa && t.related_qa.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {t.related_qa.map((qa: number) => (
                        <span key={qa} className="px-1.5 py-0.5 rounded text-xs font-medium text-white" style={{ backgroundColor: QA_COLORS[qa] }}>QA{qa}</span>
                      ))}
                    </div>
                  )}
